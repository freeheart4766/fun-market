...
let register = () => {
    ...
    if(checkUsernameAndPassword && password2 != null && loginAndRegister.verifyCode != null){
        axios.post(`${unificationURL}/seller/register`,
                    loginAndRegister,
...
//异步发送请求，检测用户名是否占用
let usernameOccupy = () => {
    axios.get(`${unificationURL}/seller/usernameOccupy/${loginAndRegister.username}`)
    .then((response) => {
        if(response.data.code != 200){
    ...
    <div>
        ...
        <div id="main">
            <div id="register-area">
                ...
                <button id="register-button" @click="register()">商家注册</button>
            </div> 
        </div>
    </div>
</template>
...
#main {
    width: 100%;
    height: 420px;
    background-image: url(../assets/seller-register-background.png);
