...
//异步发送请求，检测用户名是否存在
let existUsername = () => {
    axios.get(`${unificationURL}/seller/existUsername/${loginAndRegister.username}`)
    .then((response) => {
...
let login = () => {
    ...
    if(checkUsernameAndPassword && loginAndRegister.verifyCode != null){
        axios.post(`${unificationURL}/seller/login`,
                    loginAndRegister,
    ...
    <div>
        ...
        <div id="main">
            <div id="login-area">
                ...
                <button id="login-button" @click="login()">商家登录</button>
                <div id="router-area">
                    <router-link class="router-to" to="/sellerRegister">立即注册</router-link>
                </div>
            </div> 
        </div>
    </div>
...
#main {
    width: 100%;
    height: 400px;
    background-image: url(../assets/seller-login-background.png);
