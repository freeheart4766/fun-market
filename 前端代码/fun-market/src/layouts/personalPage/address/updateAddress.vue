...
//修改地址信息
let updateReceivingInfo = () => {
    axiosInstance.put('/receivingInfo/updateReceivingInfo',receivingInfo)
    .then((response) => {
        if(response.data.code == 200){
            successEasy('修改','收货地址信息已修改');
            toAddress();
        }else{
            error('修改','请重新提交');
        }
...
//根据receivingId获取需要修改的地址信息
let getReceivingInfo = () => {
    let receivingId =  defineSignIds.getReceivingInfoId
    axiosInstance.get(`/receivingInfo/getOneReceivingInfo/${receivingId}`)
    .then((response) => {
        receivingInfo.receivingId = receivingId;
        receivingInfo.receiverName = response.data.data.receiverName;
        receivingInfo.area = response.data.data.area;
        area.value = receivingInfo.area.split(",");
        receivingInfo.detailAddress = response.data.data.detailAddress;
        receivingInfo.telephone = response.data.data.telephone;
        receivingInfo.isDefault = response.data.data.isDefault;
    })
    .catch((errors) => {
        console.log(errors);
    })
...
<template>
    <div class="address-area">
        ...
        <button class="button" @click="updateReceivingInfo()">保存</button>
    </div>
</template>
