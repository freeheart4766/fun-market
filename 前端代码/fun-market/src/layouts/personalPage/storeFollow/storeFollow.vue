<script setup>
import { storeInfo } from '../../../utils/pojo.js'
import { useRouter } from 'vue-router';
let router = useRouter();
let toStore = () => {
    router.push('/store');
}
</script>

<template>
    <div id="storeFollow-area">
        <div id="select-area">
            <input id="input-area" placeholder="输入店铺名称">
            <div id="select-button">搜索</div>
        </div>
        <div id="store-area">
            <el-scrollbar height="535px" always="true">
                <div class="store-single">
                    <div class="store-area">
                        <img class="store-picture" v-bind:src="storeInfo.portraitPath">
                        <p class="store-name">{{ storeInfo.storeName }}</p>
                        <div class="store-opertion-area">
                            <div class="store-opertion store-into" @click="toStore()">
                                <svg t="1706262707507" class="store-opertion-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2522">
                                    <path d="M940.960749 425.143816c0.093121-17.86488-2.838651-35.791158-8.207939-52.113915-0.061398-0.463558-0.153496-0.89437-0.246617-1.26583-0.370437-1.788739-0.86367-3.517103-1.573845-5.183046L858.609159 186.54565c-12.527314-37.334303-48.50369-60.228733-90.589205-60.32083L260.369006 126.22482c-42.641169 0-75.778894 22.678512-87.781252 58.531068L94.833879 368.463909c-0.370437 1.079588-0.771573 2.684132-1.110287 4.319375-5.646604 17.339924-8.485255 35.143405-8.485255 53.008284 0.047072 47.364751 19.658735 91.617627 53.040007 123.1856l0.214894 269.90315c0 43.50484 35.452443 78.895885 79.049381 78.895885l580.622914-0.370437c43.535539-0.093121 78.957283-35.57524 78.957283-79.111802l-0.058328-260.299421C917.076782 526.08888 940.986331 477.395878 940.960749 425.143816zM798.072411 835.695287l-580.529793 0.370437c-9.564843 0-17.339924-7.713682-17.339924-17.217127l-0.185218-232.914724c16.764825 5.293563 35.163871 7.983835 55.045687 7.983835 50.632167-0.153496 97.531314-22.400173 129.496329-60.47535 31.81152 37.766139 78.340229 59.858296 128.880299 60.166311 50.138934-0.401136 96.543823-22.586414 128.200824-60.32083 31.965016 38.075177 78.926584 60.228733 129.805368 60.228733 15.559371-0.077771 30.233582-1.77646 43.916209-5.060249l0.051165 229.867318C815.413358 827.889507 807.606555 835.664587 798.072411 835.695287zM823.065641 520.114818c-13.977339 7.652284-31.410384 11.570523-51.68208 11.69332-37.58092 0-71.828932-19.068288-91.823312-51.434439-1.419326-3.055592-3.795443-8.114818-8.238638-12.989849-5.090948-5.708002-14.346753-12.464893-29.588899-12.464893-12.279674 0-23.418362 5.028527-29.712719 12.743232-4.16588 4.689812-6.448876 9.348925-8.084119 12.742209-19.74674 31.966039-53.718459 51.218522-90.527806 51.496861-37.272905-0.215918-71.335698-19.407002-91.206258-51.619658-1.388627-2.838651-3.703345-7.621585-7.312546-11.663644-14.502295-17.309224-46.960545-16.414855-59.610656-1.326205-4.659113 5.090948-7.096628 10.212596-8.670473 13.700023-19.931958 31.965016-54.211692 51.126424-91.607394 51.249221-19.931958 0-36.994566-3.734044-50.755987-11.07729l-0.030699 0c0 0 0 0-0.030699 0-35.297924-18.789948-57.235562-55.32198-57.266261-95.309716 0-11.81714 1.974981-23.727401 5.92392-35.328623 0.277316-0.832971 0.523933-1.759063 0.740874-2.715855l76.581166-181.054589c1.573845-4.628414 6.325056-18.72855 30.144554-18.72855l507.805468 0c9.780761 0.586354 26.628474 2.313695 32.613792 19.931958l71.706135 178.555675c0.278339 1.295506 0.617054 2.529613 0.926092 3.547803 3.980661 11.631922 5.954619 23.449062 5.954619 35.175127C879.375112 464.761116 857.80791 501.107929 823.065641 520.114818z" fill="#000000" p-id="2523"></path>
                                </svg>
                                <p class="store-opertion-description">进入店铺</p>
                            </div>
                            <div class="store-opertion">
                                <svg t="1706263187029" class="store-opertion-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="7207">
                                    <path d="M182.656 130.432l654.4 654.464-50.304 50.368-133.376-133.376-143.488 143.808L208 543.36a213.568 213.568 0 0 1 1.216-285.568L132.288 180.8l50.368-50.368z m77.312 364.16l249.92 250.304 93.12-93.376-343.36-343.36a142.464 142.464 0 0 0 0.32 186.432z m543.36-244.416a213.632 213.632 0 0 1 8.32 293.184l-58.176 58.24-50.304-50.304 56.576-56.704a142.336 142.336 0 0 0-202.24-199.552l-47.552 42.624-47.616-42.56a140.736 140.736 0 0 0-41.088-25.792L341.184 189.248a213.248 213.248 0 0 1 168.704 52.8 213.504 213.504 0 0 1 293.44 8.128z" p-id="7208" fill="#000000"></path>
                                </svg>
                                <p class="store-opertion-description">取消关注</p>
                            </div>
                        </div>
                    </div>
                    <div class="store-goods">
                        <img class="store-goods-picture" v-bind:src="goodsShowPicture.pitcureOnePath">
                        <p class="store-goods-price">￥{{ goodsSingleTypes.goodsPrice }}</p>
                    </div>
                </div>
            </el-scrollbar>
        </div>
    </div>
</template>

<style scoped>
#storeFollow-area {
    width: 830px;
    height: 590px;
    background-color: white;
    margin-left: 20px;
    margin-top: 20px;
    padding-top: 20px;
    border-radius: 10px;
}
#select-area {
    width: 290px;
    height: 30px;
    margin-left: 20px;
    padding-top: 5px;
    border: 1px black solid;
}
#input-area {
    width: 200px;
    height: 25px;
    border: none;
    outline: none;
    padding-left: 10px;
    float: left;
}
#select-button {
    width: 70px;
    height: 21px;
    font-size: 13px;
    text-align: center;
    background-color: rgb(235, 230, 230);
    border: 1px rgb(163, 162, 162) solid;
    padding-top: 3px;
    margin-left: 5px;
    cursor: pointer;
    float: left;
}
#store-area {
    height: 535px;
    margin-top: 20px;
}
.store-single {
    width: 790px;
    height: 175px;
    margin-left: 20px;
    margin-bottom: 20px;
    border: 2px rgb(220, 215, 215) solid;
}
.store-area {
    width: 250px;
    height: 175px;
    border-right: 2px rgb(220, 215, 215) solid;
    float: left;
}
.store-picture {
    width: 100px;
    height: 100px;
    margin-left: 15px;
    margin-top: 15px;
    border: 1px rgb(220, 215, 215) solid;
    float: left;
}
.store-name {
    width: 115px;
    height: 45px;
    margin-left: 10px;
    margin-top: 30px;
    float: left;
}
.store-opertion-area {
    width: 250px;
    height: 50px;
    margin-top: 8.5px;
    background-color: rgb(240, 236, 236);
    float: left;
}
.store-opertion {
    width: 124px;
    height: 50px;
    border-top: 2px rgb(220, 215, 215) solid;
    cursor: pointer;
    float: left;
}
.store-into {
    border-right: 2px rgb(220, 215, 215) solid;
}
.store-opertion-icon {
    width: 25px;
    margin-top: 10px;
    margin-left: 15px;
    float: left;
}
.store-opertion-description {
    margin-top: 11px;
}
.store-opertion-description:hover {
    color: rgb(255, 25, 0);
}
.store-goods {
    width: 133px;
    height: 175px;
    border-right: 2px rgb(220, 215, 215) solid;
    float: left;
}
.store-goods-picture {
    width: 100px;
    height: 100px;
    margin-left: 17px;
    margin-top: 20px;
}
.store-goods-price {
    width: 100px;
    font-size: 18px;
    font-weight: bolder;
    margin-left: 25px;
    margin-top: 10px;
}
</style>