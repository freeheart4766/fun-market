...
let uploadPicture = () => {
    if(passMuster.value == 1 && goodsCoverFile.value){
        ...
        axiosInstance.post(`/goods/updateGoodsPicture/${goods.goodsId}`,formData)
        .then((response) => {
...
let getGoods = () => {
    ...
    axiosInstance.get(`/goods/getGoods/${goods.goodsId}`)
    .then((response) => {
        goods.goodsName = response.data.data.goodsName;
        goods.goodsType = response.data.data.goodsType;
        goods.goodsInventory = response.data.data.goodsInventory;
        getPicture(response.data.data.goodsCoverPath);
    })
}
//获取图片
let getPicture = (picturePath : string) => {
...
//提交信息
let submit = () => {
    axiosInstance.put('/goods/updateGoods',goods)
    .then((response) => {
        if(response.data.code == 200){
            uploadPicture();
            success('修改','商品信息页',toGoods())
        }
    })
}
onMounted(() => {
...
<template>
    <div class="add-goods-area">
        <div class="page-top">
            <p class="page-top-content">修改商品信息</p>
